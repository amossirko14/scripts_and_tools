...
port = 3306
load_host = "10.1.1.27"
load_sock = "/var/lib/mysql/mysql.sock"
...
def do_load_ext(db_num, dest_db_name, src_tbl_pref, dest_tbl_pref, uid, items, items_hex):
    ...
    start_time = get_now_secs()
    items.insert(0, uid)
    conn = MySQLdb.connect(host=load_host, user=user, passwd=passwd, db=dest_db_name, port=port,
                           unix_socket=load_sock, local_infile=True, charset='utf8')
    with conn:
        cur = conn.cursor()
        for index in range(db_num):
            ...
            sql3 = " load data infile \"%s\" into table %s character set utf8 ( %s ) " % (src_f, tmp_tbl, combine1)
            # print(sql3)
            cur.execute(sql3)
            # cur.commit()
            ...
            with_hex_fmt_1 = " %s = unhex(" + "%s)"
            combine2_1 = join2(items_hex, with_hex_fmt_1, " , ")
            sql4_1 = "update %s set %s" % (dest_tbl, combine2_1)
            print(sql4_1)
            cur.execute(sql4_1)
...
def do_migrate():
    ...
    do_select_ext(500, "reg_usrinfor", "onlineinfor%s", "usrno", ["usernewno", "shapsw", "usrname"], ["nickname"],
                  dest_db_num)
    do_load_ext(dest_db_num, dest_db, "onlineinfor%s", dest_tb_pref, dest_uid, ["usernewno", "shapsw", "usrname"],
                ["nickname"])
